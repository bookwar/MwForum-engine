...
# It's not meant as a full backup tool and does not export complete data.
...
if (!-f "$path/minimal.css") {
	open my $fh, ">", "$path/minimal.css" or die "Opening file for writing failed";
...
{
	# Open file
	open my $fh, ">", "$path/forum.html" or die "Opening file for writing failed";
...

{
	...
	while ($boardSth->fetch()) {
		...
		if ($updateTime > $fileTime) {
			# Open file
			open my $fh, ">", $file or die "Opening file for writing failed";
