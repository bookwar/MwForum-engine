...
if (!$submitted || @{$m->{formErrors}}) {
	...
	if ($ban) {
		...
		print
			...
			"<input type='text' class='fcs' name='reason' size='80'",
			" autofocus='autofocus' value='$reasonEsc'/></label>\n",
			"<label class='lbw'>Internal Reason (shown to admins only)\n",
			"<input type='text' name='intReason' size='80' value='$intReasonEsc'/></label>\n",
			"<label class='lbw'>Duration (in days, 0 = unlimited)\n",
			"<input type='text' name='duration' size='4' maxlength='3' value='$duration'/></label>\n",
			"</fieldset>\n",
			"<fieldset>\n",
