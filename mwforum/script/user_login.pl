...
if ($submitted) {
	...
	if ($action eq 'login') {
		...
		if (!@{$m->{formErrors}}) {
			# Set cookies
			my $passwordMd5 = $m->md5($password . $dbUser->{salt});
			$m->setCookie('login', "$dbUser->{id}-$passwordMd5", !$remember);
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		...
		"<input type='text' class='fcs' name='userName' size='40' maxlength='50'",
		" autofocus='autofocus' required='required' value='$userNameEsc'/></label>\n",
		"<label class='lbw'>$lng->{lgiLoginPwd}\n",
		"<input type='password' name='password' size='40' maxlength='15' required='required'/>",
		"</label>\n",
		"</fieldset>\n",
		"<fieldset>\n",
	...
	if (!$cfg->{authenPlg}{login}) {
		...
		print
			...
			"<div class='ccl'>\n",
			"<label class='lbw'>$lng->{lgiLoginName}\n",
			"<input type='text' name='userName' size='40' maxlength='50' required='required'",
