...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		...
		"<input type='text' class='fcs' name='userName' size='40' maxlength='$cfg->{maxUserNameLen}'",
	...
	if (!$cfg->{noEmailReq}) {
		print
			...
			"<input type='email' name='emailV' size='40' maxlength='100'",
			" required='required' value='$emailVEsc'/></label>\n"
	}
	else {
		print
			"<label class='lbw'>$lng->{regRegPwd}\n",
			"<input type='password' name='password' size='20' maxlength='15'/></label>\n",
			"<label class='lbw'>$lng->{regRegPwdV}\n",
			"<input type='password' name='passwordV' size='20' maxlength='15'/></label>\n"
			if $cfg->{noEmailReq};
	}
	...
	print
		...
		"<input type='text' name='extra1' size='40' maxlength='100' value='$extra1Esc'/></label>\n"
		if $cfg->{regExtra1};
	
	print
		"<label class='lbw'>$cfg->{longExtra2}\n",
		"<input type='text' name='extra2' size='40' maxlength='100' value='$extra2Esc'/></label>\n"
		if $cfg->{regExtra2};
	
	print
		"<label class='lbw'>$cfg->{longExtra3}\n",
		"<input type='text' name='extra3' size='40' maxlength='100' value='$extra3Esc'/></label>\n"
		if $cfg->{regExtra3};
