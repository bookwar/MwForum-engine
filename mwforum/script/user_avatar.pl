...
if (!$submitted || @{$m->{formErrors}}) {
	...
	$m->printHints([$cfg->{avatarResize}
		? $m->formatStr($lng->{avaUplResize}, { bytes => $cfg->{maxAttachLen} })
	...
	if ($cfg->{avatarUpload}) {
		...
		if (!$avatar || $avatar =~ /[\/:]/) {
			print
				"<label class='lbw'><input type='file' class='fcs' name='file' size='80'",
	...
	if ($cfg->{avatarGravatar}) {
		...
		print
			...
			"<input type='email' class='fcs' name='gravatarEmail' size='80'",
