...
if (!$submitted || @{$m->{formErrors}}) {
	...
	my $label = $m->formatStr($lng->{attUplFile}, { bytes => $cfg->{maxAttachLen} });
	print	
		...
		"<input type='file' class='fcs' name='file' size='80' autofocus='autofocus'/></label>\n",
		"<label class='lbw'>$lng->{attUplCapt}\n",
		"<input type='text' name='caption' size='80' maxlength='100' value=''/></label>\n",
		"</fieldset>\n";
	...
	for my $attach (@$attachments) {
		...
		print 
			"<label class='lbw'>$lng->{attUplCapt}\n",
			"<input type='text' name='caption' size='80' maxlength='100' value='$caption'/></label>\n",
