...
my @ua = (
	'Gecko', 
		'Firefox', 
			'Firefox 5.0+', 'Firefox 4.0', 'Firefox 3.5', 'Firefox 3.0',
	'MSIE', 
		'MSIE 10.0', 'MSIE 9.0', 'MSIE 8.0', 'MSIE 7.0', 'MSIE 6.0',
	'WebKit', 
		'Chrome', 
		'Safari', 
			'Safari 5.0', 'Safari 4.0', 'Safari 3.0', 
	'Opera', 
		'Opera 11.0', 'Opera 10.5', 'Opera 10.0',
	'Other',
);
...
while ($sth->fetch()) {
	$users++;
	if (index($ua, 'Windows') > -1) { 
		$os{'Windows'}++;
		if    (index($ua, 'Windows NT 6.1') > -1) { $os{'Windows 7'}++ }
		elsif (index($ua, 'Windows NT 6.0') > -1) { $os{'Windows Vista'}++ }
		elsif (index($ua, 'Windows NT 5.2') > -1) { $os{'Windows XP'}++ }
		elsif (index($ua, 'Windows NT 5.1') > -1) { $os{'Windows XP'}++ }
		elsif (index($ua, 'Windows NT 5.0') > -1) { $os{'Windows 2000'}++ }
	}
	...
	if (index($ua, 'Opera') > -1) { 
		...
		$ua =~ /Version\/(\d+\.\d+)/;
		if ($1) {
			if    ($1 >= 11 && $1 < 12) { $ua{'Opera 11.0'}++ }
			elsif ($1 >= 10.5)          { $ua{'Opera 10.5'}++ }
			elsif ($1 >= 10)            { $ua{'Opera 10.0'}++ }
		}
		else {
			$ua =~ /Opera.(\d+\.\d+)/;
			if    ($1 >= 11 && $1 < 12) { $ua{'Opera 11.0'}++ }
		...
		if ($ua =~ /Chrome\/(\d+)/) { 
			...
			$ua{'Safari'}++;
			$ua =~ /Version\/(\d+)/;
			if    ($1 == 5) { $ua{'Safari 5.0'}++ }
			elsif ($1 == 4) { $ua{'Safari 4.0'}++ }
			elsif ($1 == 3) { $ua{'Safari 3.0'}++ }
		}
	}
	elsif ($ua =~ /(?<!like )Gecko/) { 
		$ua{'Gecko'}++;
		if ($ua =~ /Firefox\/(\d+\.\d+)/) {
			$ua{'Firefox'}++;
			if    ($1 >= 5)   { $ua{'Firefox 5.0+'}++ }
			elsif ($1 >= 4)   { $ua{'Firefox 4.0'}++ }
			elsif ($1 >= 3.5) { $ua{'Firefox 3.5'}++ }
			elsif ($1 >= 3)   { $ua{'Firefox 3.0'}++ }
		}
	}
	elsif ($ua =~ /MSIE (\d+)\.\d+/) { 
		$ua{'MSIE'}++;
