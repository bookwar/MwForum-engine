...
if ($submitted && $cfg->{blockExtSearch} && length($words)) {
	$m->{env}{method} eq 'POST' || $page > 1 or $m->error('errNoAccess');
...
# Hard age limit for performance reasons
$maxAge = $cfg->{searchMaxAge} 
	if $cfg->{searchMaxAge} && (!$maxAge || $maxAge > $cfg->{searchMaxAge});

# Split and treat keywords early to show user what we don't support anyway with indexed search
my @words = ();
my $wordsChanged = 0;
if ($cfg->{advSearch}) {
...
if ($submitted && !($wordsChanged && !$words)) {
	...
	my $wordStr = "";
	my @wordValues = ();
	if ($cfg->{advSearch} && $field eq 'body' && $words) {
		# Search with MySQL fulltext index
		for (@words) { 
			$_ = "$_*" if !/^"/;
			$_ = "+$_" if !/^-/;
		}
		$wordStr = "AND (MATCH $fieldStr AGAINST (:word0 IN BOOLEAN MODE))";
	...
	my $maxSrchResNum = $cfg->{maxSrchResNum} || 500;
	$posts = $m->fetchAllArray("
		...
		{ searchUserId => $searchUserId, groupTitle => $groupTitle, userIds => \@userIds,
			...
			maxSrchResNum => $maxSrchResNum, @wordValues });
	$postNum = @$posts;
...
if ($cfg->{forumSearch} == 1 || $cfg->{forumSearch} == 2 && $userId || $user->{admin}) {
	...
	print
		...
		"<div class='cli'>\n",
		"<label>$lng->{seaWords}\n",
		"<input type='text' class='fcs' name='words' size='20' maxlength='100'",
...
elsif ($cfg->{googleSearch} == 2) {
	# AJAX search with results on this page
	print
		"<form id='gSearchForm'>\n",
		"<div class='frm'>\n",
		"<div class='hcl'><span class='htt'>$lng->{seaGglTtl}</span></div>\n",
		"<div class='ccl'>\n",
		"<div class='cli'>\n",
		"<input type='text' class='fcs' id='gSearchQuery' size='60' $autofocus/>\n",
		"<button type='submit'><img src='http://www.google.com/uds/css/small-logo.png'",
		" style='vertical-align: bottom' alt='Google'/></button>\n",
		"</div>\n",
		"</div>\n",
		"</div>\n",
		"</form>\n\n",
		"<div id='gSearchResults'></div>\n\n",
		"<div class='frm' id='gSearchMore' style='display: none'>\n",
		"<div class='ccl'><a class='clk'>$lng->{serMore}</a></div>\n",
		"</div>\n\n",
		"<div class='frm' id='gSearching' style='display: none'>\n",
		"<div class='ccl'>$lng->{serSearching}</div>\n",
		"</div>\n\n",
		"<div class='frm' id='gSearchNotFound' style='display: none'>\n",
		"<div class='ccl'>$lng->{serNotFound}</div>\n",
		"</div>\n\n";
}
...
if ($submitted && !($wordsChanged && !$words)) {
	...
	for my $post (@$posts) {
		...
		$m->dbToDisplay({}, $post);
		$url = $m->url('topic_show', pid => $postId, hl => $hilite);
