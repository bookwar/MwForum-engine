...
my $views = [
	{	name => " Identity",
		...
		searchEsc => $percent . $m->dbEscLike($m->escHtml($search)) . $percent,
		searchUsers => sub {
	...
	{ name => "Badges",
		...
		searchEsc => $m->dbEscLike($m->escHtml($search)) . $percent,
		searchUsers => sub { 
	...
	{ name => "Groups",
		...
		searchEsc => $m->dbEscLike($m->escHtml($search)),
		searchUsers => sub { 
		...
		columns => [
			{
				...
				value => sub {
					...
					return if !$v;
					index($v, "gravatar:") == 0
						? "<img src='http://gravatar.com/avatar/" . $m->md5(substr($v, 9)) . 
...
if (length($search)) {
	if ($view->{searchUsers}) {
		$searchEsc = $view->{searchEsc};
	}
	elsif ($view->{type} eq 'int') {
		$searchEsc = int($search || 0);
...
# Get ids of users
my $users = $view->{searchUsers} ? $view->{searchUsers}({ search => $searchEsc }) : 
	$m->fetchAllArray("
		SELECT id
		FROM users AS users
		WHERE 1 = 1
...
print
	...
	" autofocus='autofocus' style='width: 100px' value='$search'/></label>\n",
