...
#------------------------------------------------------------------------------
# Format key/string-value pairs as JSON
...
sub updateUser
{
	...
	if ($env->{script} !~ /^topic_|^branch_|^post_|^poll_|^report_/ && $user->{lastTopicId}) {
...
sub deleteUser
{
	...
	# Delete avatar
	unlink "$cfg->{attachFsPath}/avatars/$delUser->{avatar}" 
...
sub setCookie
{
	...
	$http = !defined($http) || $http ? "httpOnly" : "";
...
sub printHeader 
{
	...
	print 
		...
		if $cfg->{microsummary} && $script eq 'forum_show';
	...
	print "<meta name='jsparams' content='", $m->json($jsParams), "'/>\n";
	my $jQueryUrl = $cfg->{jQueryUrl} || "$dataPath/jquery.js";
	print "<script src='$jQueryUrl'></script>\n";
	print "<script src='$dataPath/mwforum.js'></script>\n";
	...
	if ($cfg->{titleImage} && $script ne 'image_show') {
		print
...
sub printFooter 
{
	...
	print
		...
		" $VERSION &#169; 1999-2011 Markus Wichitill</p>\n\n"
		if $m->{env}{script} ne 'forum_info' && $m->{env}{script} ne 'image_show';
...
sub editToDb
{
	...
	for my $word (@{$cfg->{censorWords}}) {
		my @chars = split(//, $word);
		my $rx = join("+", @chars);
		$$subject =~ s!$rx!'*' x length($word)!egi if $$subject;
		$$body =~ s!$rx!'*' x length($word)!egi;
	}
	...
	%
		...
		elsif ($4) { "<a class='url' href='$4'>$4</a>" }
...
sub dbToDisplay
{
	...
	if ($cfg->{openUrlNewTab}) {
		$$body =~ s!<a class='url' href!<a class='url' target='_blank' href!g;
		$$sig  =~ s!<a class='url' href!<a class='url' target='_blank' href!g 
			if $$sig && $cfg->{fullSigs};
	}
	...
	if ($cfg->{videoTag} && $embed && $filter) {
		$$body =~ s%\[vid=(html|youtube|vimeo)\](.+?)\[/vid\]%
			my $srv = lc($1);	my $id = $2;
			if ($srv eq 'html') {
				return if $id !~ m!^https?://[^\s\\\[\]{}<>)|^`'"]+\z!;
				"<video src='$id' controls='controls'><p>$lng->{errUAFeatSup}</p></video>"
			}
			else {
				return if $id !~ /^[A-Za-z_0-9-]+\z/;
				my ($height, $url);
				$srv eq 'youtube' 
					?	"<iframe class='vif' width='640' height='385' src='http://youtube.com/embed/$id'></iframe>"
					: "<iframe class='vif' width='640' height='360' src='http://player.vimeo.com/video/$id'></iframe>"
			}
		%egi;
	}
	...
	if ($attachments && @$attachments) {
		...
		$$body =~ s^\[img( thb)?\]([\w.-]+\.(?:jpg|png|gif))\[/img\]^
			...
			if (exists($attachments{$fileName})) {
				...
				my $imgShowUrl = $m->url('image_show', aid => $attachments{$fileName}{id});
				...
				if ($embed) {
					($thumb || $cfg->{attachImgThb}) && (-f $thbFsPath || $m->addThumbnail($attFsPath) > 0)
						? "<a href='$imgShowUrl'><img class='amt' src='$thbUrlPath' title='$size' alt=''/></a>"
						: "<img class='ami' src='$attUrlPath' alt=''/>";
				}
				else { "<a href='$imgShowUrl'>$fileName</a> ($size)" }
			}
			else { "[$fileName]" }
		^egi;
		...
		for my $attach (@$attachments) {
			...
			my $attFsPath = "$attFsBasePath/$fileName";
			my $attUrlPath = "$attUrlBasePath/$fileName";
			my $imgShowUrl = $m->url('image_show', aid => $attach->{id});
			...
			if ($cfg->{attachImg} && $attach->{webImage} == 2 && $embed) {
				...
				$$body .= $cfg->{attachImgThb} && (-f $thbFsPath || $m->addThumbnail($attFsPath) > 0)
					? "\n<a href='$imgShowUrl'><img class='amt' src='$thbUrlPath' $title alt=''/></a>"
					: "\n<img class='ami' src='$attUrlPath' $title alt=''/>";
			}
			else {
				my $url = $attach->{webImage} ? $imgShowUrl : $attUrlPath;
...
sub dbToEmail
{
	...
	$$body =~ s!</span>!!g;
	$$body =~ s!<a class='url' href='(.+?)'>(.+?)</a>!$2 <$1>!g;
...
sub dbConnect 
{
	...
	if ($cfg->{dbDriver} eq 'mysql') {
		...
		$dbh = DBI->connect(
			...
			$cfg->{dbUser}, $cfg->{dbPassword}, 
			{ PrintError => 0, PrintWarn => 0, RaiseError => 0, AutoCommit => 1, pg_enable_utf8 => 0 })
			or $m->dbError();
		$dbh->do("SET NAMES 'utf8'");
...
sub logAction
{
	...
	$m->callPlugin($_, level => $level, entity => $entity, action => $action,
		...
		postId => $postId, extraId => $extraId) for @{$cfg->{logPlg}};
	...
	$m->dbDo("
		INSERT INTO log (	
			level, entity, action, userId, boardId, topicId, postId, extraId, logTime, ip) 
		VALUES (?,?,?,?,?,?,?,?,?,?)",
...
sub logString
{
	...
	length($string) or return 0;
	$m->dbDo("
		INSERT INTO logStrings (string) VALUES (?)", $string);
	
	return $m->dbInsertId("logStrings");
}
...
sub deletePost
{
	...
	$hasChildren = $m->fetchArray("
		SELECT 1 FROM posts WHERE parentId = ?", $postId)
	...
	$alone = !$m->fetchArray("
		SELECT 1 FROM posts WHERE id <> ? AND topicId = ?", $postId, $topicId)
		if !defined($alone);

	# Don't delete base posts that are not alone
	my $base = $m->fetchArray("
		SELECT basePostId = ? FROM topics WHERE id = ?", $postId, $topicId);
	return undef if $base && !$alone;
	...
	if ($alone) {
		...
		if ($hasChildren) {
			...
			$m->dbDo("
				UPDATE posts SET body = ? WHERE id = ?", $m->{lng}{eptDeleted}, $postId);
			$m->setLanguage();
		}
		else {
			# Delete post
...
sub deleteTopic
{
	...
	$topicExists or $m->error('errTpcNotFnd');
...
sub sendEmail
{
	...
	# Apply quoted-printable
	if ($cfg->{mailer} =~ /^(?:SMTP2|sendmail|mail)\z/) {
	...
	if ($cfg->{mailer} eq 'SMTP') {
		...
		MwfSendmail::sendmail(
			smtp => $cfg->{smtpServer}, From => $from, To => $to, Subject => $subject,
		...
		Mail::Sender->new()->MailMsg({
			...
			auth => $cfg->{esmtpAuth}, authid => $cfg->{esmtpUser}, authpwd => $cfg->{esmtpPassword},
