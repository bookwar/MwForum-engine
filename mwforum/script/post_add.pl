...
if ($add) {
	...
	# Process text
	my $post = { body => $body, rawBody => $rawBody };
	...
	if (!@{$m->{formErrors}}) {
		...
		!$m->fetchArray(" 
			...
			WHERE topicId = :topicId
				...
				AND body = :body
			LIMIT 1",
			{ topicId => $topicId, parentId => $parentId, userId => $userId, now => $m->{now}, 
				body => $post->{body} })
			or $m->error('errDupe');
		...
		$m->dbDo("
			UPDATE topics SET postNum = postNum + 1, lastPostTime = ? WHERE id = ?", 
			$m->{now}, $topicId);
		$m->dbDo("
		...
		if ($recvUser && $recvUser->{notify} && $recvUser->{id} != $userId && $approved
			&& !$topicReply && $m->boardVisible($board, $recvUser)) {

			# Check whether receiver ignores poster
			my $ignored = $m->fetchArray("
				SELECT 1 FROM userIgnores WHERE userId = ? AND ignoredId = ?", 
				$recvUser->{id}, $userId);

			if (!$ignored) {
				# Add notification message
				my $url = "topic_show$m->{ext}?pid=$postId";
				$m->addNote('pstAdd', $recvUser->{id}, 'notPstAdd', 
					usrNam => $postUserName, pstUrl => $url);
