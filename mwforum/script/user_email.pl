...
if ($submitted) {
	...
	$email eq $emailV || $user->{admin} or $m->formError('errEmlDiffer');
	$m->checkEmail($email);
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		...
		"<input type='email' class='fcs' name='email' size='40' maxlength='100'",
		" autofocus='autofocus' required='required' value='$emailEsc'/></label>\n",
