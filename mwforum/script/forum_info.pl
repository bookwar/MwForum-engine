...
if (!$details) {
	...
	if ($cfg->{topicHits}) {
		my $hitNum = $cfg->{topicHits} ? $m->fetchArray("
			SELECT SUM(hitNum) FROM topics") : 0;
		print "<tr class='crw'><td class='hco'>$lng->{fifStsHitNum}</td><td>$hitNum</td></tr>\n";
	}
