...
if ($submitted || $openIdMode) {
	...
	# Use own nonce in addition to consumer_secret/oic.time, which isn't really a nonce
	my $nonce = $m->randomId();
	...
	my $secretSub = sub {
		my $time = shift();
		my $secret = $cache->get("mwf.secret:$time");
		return $secret if $secret;
		$cache->set("mwf.secret:$time", $nonce);
		return $nonce;
	};
	my $csr = Net::OpenID::Consumer->new(
		cache => $cache,
		ua => LWPx::ParanoidAgent->new(timeout => 5),
		args  => sub { $MwfMain::MP ? $m->{apr}->param($_[0]) : $m->{cgi}->param($_[0]) },
		required_root => $cfg->{baseUrl},
		consumer_secret => $secretSub,
		debug => $cfg->{debug} ? 1 : 0,
	);
	...
	if ($submitted && !$openIdUrl) {
		...
		if ($csr->user_cancel()) {
			...
			# Verify and delete own nonce, since oic.time is reusable
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		...
		"<input type='text' class='fcs' id='openid_url' name='openid_url' size='60' maxlength='200'",
